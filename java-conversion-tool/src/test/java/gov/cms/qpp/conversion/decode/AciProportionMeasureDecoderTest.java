...
public class AciProportionMeasureDecoderTest {
	...
	public void decodeACIProportionMeasureAsNode() throws Exception {
		...
		assertThat("returned node should have one child node", root.getChildNodes(), hasSize(1));
		...
		assertThat("Should have Numerator", aciProportionMeasureNode.getChildNodes().get(0).getId(),
				is(TemplateId.ACI_NUMERATOR.getTemplateId()));
		assertThat("Should have Denominator", aciProportionMeasureNode.getChildNodes().get(1).getId(),
