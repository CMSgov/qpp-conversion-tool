...
public class ACIProportionNumeratorParserTest {
	...
	public void parseACIProportionNumeratorAsNode() throws Exception {
		...
		assertThat("returned node should have one child parser node", aciProportionNumeratorNode.getChildNodes().size(), is(1));
		// This is the node with the numerator value
		Node rateAggrNode = aciProportionNumeratorNode.getChildNodes().get(0);
		// Get the actual value
		String actual = (String) rateAggrNode.get("rateAggregation");
		assertThat("rate aggregation numerator should be 600", actual, is("600"));
