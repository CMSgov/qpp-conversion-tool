...
Generated Java API documentation (aggregated across core modules) is bundled with the REST API build and exposed as static content at:

```
/v3/javadoc/
```

The root path serves the aggregated Javadoc `index.html` generated by the Maven Javadoc Plugin’s `aggregate` goal. All public types from the participating modules (e.g. `commons`, `converter`, `commandline`) are merged into one coherent API tree. Package lists in the left nav and search box span every included module.

Implementation notes:
 
* Aggregation runs during the `prepare-package` phase inside the `rest-api` module using the standard `maven-javadoc-plugin`.
* Output is written to `rest-api/target/aggregated-javadoc` and then copied into `BOOT-INF/classes/public/v3/javadoc` so Spring Boot’s static resource handler serves it.
* OpenAPI JSON remains available at `/v3/api-docs` (unchanged).

Partial builds: run with `-am` so sibling modules compile first (the aggregate task needs their compiled classes and sources on the reactor). Example:

```bash
mvn -pl rest-api -am package
```
Skipping modules means their packages simply won’t appear in the aggregated docs (no failure).

You can omit all Javadoc content (e.g., for a slimmer production image) by building with:

```bash
mvn -DskipJavadocResources=true package
```

That skips aggregation and the copy step; the `/v3/javadoc/` endpoint (and subpaths) will then 404.

Legacy note: a previous implementation unpacked separate per‑module Javadoc trees under `/v3/javadoc/{module}/` with a custom landing page. This was replaced by the aggregated form to reduce maintenance and provide unified search/navigation.
